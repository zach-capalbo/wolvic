...
class HistoryDelegateImpl implements GeckoSession.HistoryDelegate {
    private ISession.HistoryDelegate mDelegate;
    private SessionImpl mSession;

    public HistoryDelegateImpl(ISession.HistoryDelegate delegate, SessionImpl session) {
        mDelegate = delegate;
        mSession = session;
    }
    ...
    public void onHistoryStateChange(@NonNull GeckoSession session, @NonNull HistoryList historyList) {
        mDelegate.onHistoryStateChange(mSession, new ISession.HistoryDelegate.HistoryList() {
            @Override
            ...
            @Override
            public List<ISession.HistoryDelegate.HistoryItem> getItems() {
                return historyList.stream().map(HistoryItemImpl::new).collect(Collectors.toList());
            }
        });
    }

    private static class HistoryItemImpl implements ISession.HistoryDelegate.HistoryItem {
