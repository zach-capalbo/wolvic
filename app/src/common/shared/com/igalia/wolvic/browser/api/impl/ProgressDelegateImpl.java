...
class ProgressDelegateImpl implements GeckoSession.ProgressDelegate {
    ISession.ProgressDelegate mDelegate;
    SessionImpl mSession;

    public ProgressDelegateImpl(ISession.ProgressDelegate delegate, SessionImpl session) {
        mDelegate = delegate;
        mSession = session;
    }
    ...
    public void onSecurityChange(@NonNull GeckoSession session, @NonNull SecurityInformation info) {
        mDelegate.onSecurityChange(mSession, new ISession.ProgressDelegate.SecurityInformation(
    ...
    private int fromGeckoSecurityMode(int securityMode) {
        switch (securityMode) {
            case SecurityInformation.SECURITY_MODE_IDENTIFIED:
                return ISession.ProgressDelegate.SecurityInformation.SECURITY_MODE_IDENTIFIED;
            case SecurityInformation.SECURITY_MODE_UNKNOWN:
                return ISession.ProgressDelegate.SecurityInformation.SECURITY_MODE_UNKNOWN;
            case SecurityInformation.SECURITY_MODE_VERIFIED:
                return ISession.ProgressDelegate.SecurityInformation.SECURITY_MODE_VERIFIED;
        }
    ...
    private int fromGeckoContentType(int contentType) {
        switch (contentType) {
            case SecurityInformation.CONTENT_BLOCKED:
                return ISession.ProgressDelegate.SecurityInformation.CONTENT_BLOCKED;
            case SecurityInformation.CONTENT_LOADED:
                return ISession.ProgressDelegate.SecurityInformation.CONTENT_LOADED;
            case SecurityInformation.CONTENT_UNKNOWN:
                return ISession.ProgressDelegate.SecurityInformation.CONTENT_UNKNOWN;
        }
