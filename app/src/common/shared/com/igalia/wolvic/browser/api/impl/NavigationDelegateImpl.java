...
class NavigationDelegateImpl implements GeckoSession.NavigationDelegate {
    ISession.NavigationDelegate mDelegate;
    SessionImpl mSession;

    public NavigationDelegateImpl(ISession.NavigationDelegate delegate, SessionImpl session) {
        mDelegate = delegate;
        mSession = session;
    }
    ...
    public GeckoResult<GeckoSession> onNewSession(@NonNull GeckoSession session, @NonNull String uri) {
        return ResultImpl.from(mDelegate.onNewSession(mSession, uri)).map(value -> {
            assert value != null;
    ...
    public GeckoResult<String> onLoadError(@NonNull GeckoSession session, @Nullable String uri, @NonNull WebRequestError error) {
        return ResultImpl.from(mDelegate.onLoadError(mSession, uri, new com.igalia.wolvic.browser.api.WebRequestError() {
            @Override
            public int code() {
                return error.code;
            }
    ...
    private ISession.NavigationDelegate.LoadRequest fromGeckoLoadRequest(LoadRequest request) {
        return new ISession.NavigationDelegate.LoadRequest(
