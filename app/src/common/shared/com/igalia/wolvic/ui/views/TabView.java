...
public class TabView extends RelativeLayout implements ISession.ContentDelegate, Session.BitmapChangedListener {
    ...
    @Override
    public void onTitleChange(@NonNull ISession session, @Nullable String title) {
        if (!mShowAddTab) {
    ...
    @Override
    public void onCloseRequest(@NonNull ISession geckoSession) {
        if (mSession.getISession() == geckoSession) {
            mDelegate.onClose(this);
        }
    }
